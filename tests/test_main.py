...
class TestImportFileMenu:
    def test_full_import_writes_merged_config(self, kubeconfig_file, tmp_path):
        src = tmp_path / "remote.yaml"
        src.write_text(yaml.dump(REMOTE_SINGLE))

        with patch("kubecontext.tools_context.KUBECONFIG_PATH", kubeconfig_file), \
             patch("kubecontext.main.questionary.text")    as mock_txt, \
             patch("kubecontext.main.questionary.confirm") as mock_conf:
            mock_txt.return_value.ask.return_value  = str(src)
            mock_conf.return_value.ask.return_value = True
            main.import_file_menu()
    ...
    def test_aborted_does_not_change_config(self, kubeconfig_file, tmp_path):
        src = tmp_path / "remote.yaml"
        src.write_text(yaml.dump(REMOTE_SINGLE))
        ...
        saved = load_kubeconfig(kubeconfig_file)
        assert "default" not in [c["name"] for c in saved["contexts"]]

    def test_nonexistent_file_does_not_write(self, kubeconfig_file, tmp_path):
        with patch("kubecontext.tools_context.KUBECONFIG_PATH", kubeconfig_file), \
             patch("kubecontext.main.questionary.text") as mock_txt, \
             patch("kubecontext.tools_context.save_kubeconfig") as mock_save:
            mock_txt.return_value.ask.return_value = str(tmp_path / "nonexistent.yaml")
            main.import_file_menu()
        mock_save.assert_not_called()

    def test_cancelled_text_does_not_write(self, kubeconfig_file):
        with patch("kubecontext.tools_context.KUBECONFIG_PATH", kubeconfig_file), \
             patch("kubecontext.main.questionary.text") as mock_txt, \
             patch("kubecontext.tools_context.save_kubeconfig") as mock_save:
            mock_txt.return_value.ask.return_value = None
            main.import_file_menu()
        mock_save.assert_not_called()

    def test_creates_backup_on_confirm(self, kubeconfig_file, tmp_path):
        src = tmp_path / "remote.yaml"
        src.write_text(yaml.dump(REMOTE_SINGLE))
    ...
    def test_overwrites_existing_context_with_same_name(self, kubeconfig_file, tmp_path):
        existing = {
            **_empty_config(),
            "clusters":  [{"name": "default", "cluster": {"server": "https://old:6443"}}],
            "contexts":  [{"name": "default", "context": {"cluster": "default", "user": "old-user"}}],
            "users":     [{"name": "old-user", "user": {}}],
            "current-context": "default",
        }
        kubeconfig_file.write_text(yaml.dump(existing))
        src = tmp_path / "remote.yaml"
        src.write_text(yaml.dump(REMOTE_SINGLE))
        ...
        saved   = load_kubeconfig(kubeconfig_file)
        cluster = next(c for c in saved["clusters"] if c["name"] == "default")
        assert cluster["cluster"]["server"] == "https://192.168.1.1:6443"
        assert len([c for c in saved["contexts"] if c["name"] == "default"]) == 1

    def test_multi_context_shows_checkbox_and_imports_selection(self, kubeconfig_file, tmp_path):
        src = tmp_path / "remote.yaml"
        src.write_text(yaml.dump(REMOTE_MULTI))
        ...
        with patch("kubecontext.tools_context.KUBECONFIG_PATH", kubeconfig_file), \
             ...
             patch("kubecontext.main.questionary.confirm")  as mock_conf:
            mock_txt.return_value.ask.return_value  = str(src)
            mock_chk.return_value.ask.return_value  = ["alpha"]
            mock_conf.return_value.ask.return_value = True
            main.import_file_menu()
    ...
    def test_multi_context_cancelled_checkbox_does_not_write(self, kubeconfig_file, tmp_path):
        src = tmp_path / "remote.yaml"
        src.write_text(yaml.dump(REMOTE_MULTI))
    ...
    def test_empty_file_does_not_prompt_confirm(self, kubeconfig_file, tmp_path):
        src = tmp_path / "empty.yaml"
        src.write_text(yaml.dump(_empty_config()))
